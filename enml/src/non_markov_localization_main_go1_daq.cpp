...
float kMaxPointCloudRange = 20.0;
...
string maps_dir_header_ = ros::package::getPath("go1_autonomy");
...
const char* robot_config_ = "go1.lua";
...
void CheckPointExclusion(int& i){
  if(segmented_scan_.intensities[i] == 69){
    ++i;
    CheckPointExclusion(i);
    std::cout << "CheckPointExclusion" << std::endl;
  }
}
...
void PublishSegmentedScan(
    ...
    segmented_scan_.angle_min = -3.14;
    segmented_scan_.angle_max = 3.14;
    segmented_scan_.angle_increment = (segmented_scan_.angle_max - segmented_scan_.angle_min)
        / segmented_scan_.ranges.size();

    int i = 0;
    for (size_t j = 0; j < segmented_scan_.ranges.size(); ++j) {

      if(segmented_scan_.intensities[j] == 69) continue;

      switch (classifications[end_pose][i]) {
        case NonMarkovLocalization::kLtfObservation : {
          segmented_scan_.intensities[j] = 1.0;
        } break;
        case NonMarkovLocalization::kStfObservation : {
          segmented_scan_.intensities[j] = 0.5;
        } break;
        case NonMarkovLocalization::kDfObservation : {
          segmented_scan_.intensities[j] = 0.0;
        } break;
      }
      ++i;
    }

    segmented_scan_.header.stamp = ros::Time::now();
    segmented_scan_.header.frame_id = "base";//base_footprint";
    segmented_scan_pub_.publish(segmented_scan_);
    for(int k=0; k < 897; ++k ){
        segmented_scan_.intensities[k] = 0;
        segmented_scan_.ranges[k] = 0;
    }
}
...
void LaserCallback(const sensor_msgs::LaserScan& laser_message) {
  ...
  for (size_t i = 0; i < laser_message.ranges.size(); ++i) {
    if (laser_message.ranges[i] < kMinPointCloudRange ||
        ...
        segmented_scan_.ranges[i] = 0.;
        segmented_scan_.intensities[i] = 69;
        continue;
    }
    segmented_scan_.ranges[i] = laser_message.ranges[i];
    segmented_scan_.intensities[i] = 0;
    const float angle = laser_message.angle_min +
        laser_message.angle_increment * static_cast<float>(i);
    float range = laser_message.ranges[i];
    point_cloud.push_back(
...
int main(int argc, char** argv) {
  ...
  segmented_scan_.ranges.resize(897);
  segmented_scan_.intensities.resize(897);
  segmented_scan_.range_min = 0.3;
  segmented_scan_.range_max = 15.0;
